                  ...
                  for max_grad_norm in "${max_grad_norm_values[@]}"; do
                    for per_device_train_batch_size in "${per_device_train_batch_size_values[@]}"; do
                      for epoch in "${num_train_epochs[@]}"; do
                        ...
                        bash run_unsloth_3_2.sh "$lora_r" "$lora_alpha" "$lora_dropout" "$lora_target_modules" "$use_4bit_quantization" \
                        "meta-llama/Llama-3.2-1B-Instruct" "$max_seq_len" "$learning_rate" "$lr_scheduler_type" "$warmup_ratio" \
